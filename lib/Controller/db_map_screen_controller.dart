...
class DBMapScreenController extends GetxController{
  ...
  void setUpMapData() async {
    ...
    originLatLong = PointLatLng(position.latitude,position.longitude);
    ...
    routesList.addData(PolylineWayPoint(location: "${originLatLong.latitude},${originLatLong.longitude}"), loopingLatLong, mapDataForNavigation.allAddress[0]);
  ...
  Future<void> uploadDBData(LatLng myLatLong) async {
    CollectionReference users = FirebaseFirestore.instance.collection(Helper.deliveryBoyForFB);
    try {
      if(navigationStarted.value){
        await users.doc(myUserId.toString()).set({
          'bearing': mainBearing,
          'lat': myLatLong.latitude.toDouble(),
          'long': myLatLong.longitude.toDouble(),
        });
      }
    } catch (e) {
  ...
  calculateDistance()async{
    if(finalDestinationReached.value){
      return;
    }
    if(tempCounter == 0){
      final googleMapsService = GoogleMapsService();
      final result = await googleMapsService.getTravelTime(
        origin: myLatLong.latitude.toString()+","+myLatLong.longitude.toString(),
        destination: destinationLatLong.latitude.toString()+","+destinationLatLong.longitude.toString(),
      );
  ...
  void reDrawPoliLineOnMap(LatLng locationsData)async {
    if(finalDestinationReached.value){
      return;
    }
    PointLatLng originTemp = PointLatLng(locationsData.latitude,locationsData.longitude);
  ...
  updateLocationAndUi(LocationData newLocationData) async {
    debugPrint("location update ==> onLocationChanged");
      ...
      uploadDBData(myLatLong);
      reDrawPoliLineOnMap(myLatLong);
    ...
    animateCar(myLatLong);
      ...
      double calcDestinationInMtr = Global.calculateDistanceInMeter(myLatLong.latitude, myLatLong.longitude,destinationLatLong.latitude,destinationLatLong.longitude);
      if(calcDestinationInMtr <= 20){
        Global.showToast("Your Final Destination Reached");
        distanceInKMStr = "0 km";
        durationOfDistance = "0 Mins";
        navigationStarted(false);
        finalDestinationReached(true);

        //start this code for change status of auto delivered
        changeStatusOfParcel("تم التسليم"); // Delivered
        sendNotification(false);
        //end this code for change status of auto delivered

        showDeliveredDialog(tempContext);
